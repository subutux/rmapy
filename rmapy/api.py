...
class Client(object):
  ...
  def request(self, method: str, path: str, data=None, body=None,
      headers=None, params=None, stream=False) -> requests.Response:
    ...
    Returns:
      ...
      the server.
    """
    maybe_token = self.token_set.get('usertoken', '')
    headers = {
      'contentType': 'application/json',
      'Authorization': f'Bearer {maybe_token}',
      **(headers or {}),
    }

    log.debug(path, headers)
    r = requests.request(
      method, path, json=body, data=data, headers=headers, params=params, stream=stream)
    return r

  def register_device(self, code: str):
    """Registers a device on the Remarkable Cloud.
    code: unique one-time code: https://my.remarkable.com/connect/remarkable
    """
  ...
  def list_docs(self):
    response = self.request('GET', const.URL_LIST_DOCS, params={'withBlob': 1})
    return response
  ...
  def renew_token(self):
    ...
    if response.ok:
      ...
      log.error('Error setting user token: %s', response)
